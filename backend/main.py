...
app = FastAPI(
    ...
    description="Production-ready backend with SlowAPI rate limiting & 90s concurrency queueing.",
    version="1.4.0"
)
...
async def generate_endpoint(request: Request, req: GenerateRequest):
    """Production endpoint with SlowAPI rate limiting (10 req/min) & Semaphore queueing (max 10 concurrent, 90s timeout)."""
    if not req.prompt and not req.template_id:
        raise HTTPException(status_code=400, detail="Prompt or template_id is required.")

    try:
        # Queue request through Semaphore with a 90-second timeout
        ...
        raise HTTPException(
            status_code=504,
            detail="Generation request timed out after 90 seconds. Please try again with a shorter prompt."
        )
